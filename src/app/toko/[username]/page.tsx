...
const SAMPLE_PRODUCTS = [
  { id: 1, emoji: '🥟', name: 'Siomay Frozen Ayam', price: 45000, unit: 'isi 20 pcs', bg: '#fff7ed' },
  { id: 2, emoji: '🍜', name: 'Bakso Sapi Premium', price: 55000, unit: 'isi 25 pcs', bg: '#f0fdf4' },
  { id: 3, emoji: '🦐', name: 'Udang Crispy Frozen', price: 65000, unit: 'isi 500gr', bg: '#fefce8' },
  { id: 4, emoji: '🥩', name: 'Nugget Ayam Homemade', price: 42000, unit: 'isi 300gr', bg: '#fdf2f8' },
]
...
export default function StorePage({ params }: { params: { username: string } }) {
  ...
  const cartItems = SAMPLE_PRODUCTS.filter(p => cart[p.id])
      ...
      <div className="max-w-[440px] mx-auto pb-[100px]">
        ...
        <div className="bg-white border-b border-gray-200 mb-4">
          ...
          <div className="flex justify-center">
            <div className="w-[72px] h-[72px] bg-gradient-to-br from-green-600 to-green-400 rounded-[20px] flex items-center justify-center text-4xl border-[3px] border-white shadow-card-lg -mt-9 relative z-10">
              🥟
            </div>
          </div>
          ...
          <div className="text-center px-5 pt-3 pb-5">
            ...
            <div className="flex gap-1.5 justify-center flex-wrap">
              {['✓ Terpercaya', '🚚 COD & Ongkir', '⚡ Respon Cepat'].map((t) => (
                <div key={t} className="text-[10px] font-bold text-green-600 bg-green-50 border border-green-200 rounded-full px-2.5 py-1">
                  {t}
                </div>
              ))}
            </div>
        ...
        <div className="px-3.5 mb-5">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {SAMPLE_PRODUCTS.map(p => (
              ...
              >
                {/* Product Image */}
                <div className="w-full aspect-square flex items-center justify-center text-5xl" style={{ background: p.bg }}>
                  {p.emoji}
                </div>

                {/* Check Mark */}
                <div className={`absolute top-2 right-2 w-[22px] h-[22px] rounded-full bg-green-600 text-white text-[11px] flex items-center justify-center transition-all duration-200 ${cart[p.id] ? 'opacity-100 scale-100' : 'opacity-0 scale-[0.6]'}`}>
                  ✓
                </div>
            ...
            <div className="flex items-center justify-between mb-2.5">
              <div>
                ...
                <div className="text-[11px] text-gray-500 mt-0.5">
                  {cartItems.map(p => `${p.emoji} ×${cart[p.id]}`).join('  ')}
                </div>
              </div>
